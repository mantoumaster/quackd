...
**Or put a stepper in front of the model.** `--jev` is optional, off by default and needs no change to any of the ten steps above. With `--jev shadow` a run is exactly the run it would have been, and the transcript also records what a classifier would have chosen on each turn, which is how you find out whether it is worth switching on for your bench. With `--jev on` it answers the turns whose answer is a choice among calls the arm already has, and every joint angle is still the model's. Needs `quackd[jev]` and `TYPESAFE_API_KEY` ([docs/jev.md](docs/jev.md)).

```bash
quackd run arm-grip-check --robot arm-01 --by-hand --jev shadow
```
...
uvx --from "quackd[microduck,openai]" quackd run find-and-kick --provider ollama --model qwen3:8b   # local model, no key
...
| Discrete stepper (`--jev`, `quackd[jev]`) | ✅ implemented and tested offline against a stub SDK, with the classification of every verb of every shipped body frozen in a test, 🧪 never run against TypeSafe's API and never on hardware, so the speed and cost figures in its page are an estimate rather than a measurement. Off by default. `--jev shadow` exists to measure it before anybody switches it on ([docs/jev.md](docs/jev.md)) |
...
**An optional stepper in front of the model.** `--jev` lets [TypeSafe's Jev](https://docs.typesafe.ai/introduction) take the turns whose answer is a choice among calls this body already has. It is a different kind of model rather than a cheaper one of the same kind: it emits no tokens, so where an LLM writes you a verb and quackd parses it, Jev scores the verbs you gave it and hands back which one with a calibrated probability, and there is nowhere in that answer for a number to come from. It sits inside *think* and nowhere else: no adapter knows about it, the executor is unchanged, and which verbs it may answer is computed from each tool's own JSON schema, so a verb with a number in it is never one of them. It is off unless a run asks for it, and a turn it answers never enters the model's history, because none of it is anything the model said. [docs/jev.md](docs/jev.md) and [ADR-0040](docs/adr/0040-a-discrete-stepper-in-front-of-the-model.md).
...
Requirements: Python 3.11 or newer and [`uv`](https://docs.astral.sh/uv/). Windows, macOS and Linux. No GPU. The core is about 250 MB (OpenCV is most of it) and it is the loop, the executor, the contract and the cartoon arena, with no robot in it.
...
uvx --from "quackd[microduck]" quackd run --goal "find the ball and kick it" --provider fake
...
|---|---|---|---|
| Claude | `quackd[anthropic]` | `ANTHROPIC_API_KEY` | `uvx --from "quackd[microduck,anthropic]" quackd run find-and-kick --provider anthropic` |
| OpenAI | `quackd[openai]` | `OPENAI_API_KEY` | `uvx --from "quackd[microduck,openai]" quackd run find-and-kick --provider openai` |
| Gemini | `quackd[gemini]` | `GEMINI_API_KEY` | `uvx --from "quackd[microduck,gemini]" quackd run find-and-kick --provider gemini` |
| Grok | `quackd[grok]` | `XAI_API_KEY` | `uvx --from "quackd[microduck,grok]" quackd run find-and-kick --provider grok` |
| Mistral | `quackd[mistral]` | `MISTRAL_API_KEY` | `uvx --from "quackd[microduck,mistral]" quackd run find-and-kick --provider mistral` |
| DeepSeek | `quackd[deepseek]` | `DEEPSEEK_API_KEY` | `uvx --from "quackd[microduck,deepseek]" quackd run find-and-kick --provider deepseek` |
| Cohere | `quackd[cohere]` | `COHERE_API_KEY` | `uvx --from "quackd[microduck,cohere]" quackd run find-and-kick --provider cohere` |
| Qwen | `quackd[qwen]` | `DASHSCOPE_API_KEY` | `uvx --from "quackd[microduck,qwen]" quackd run find-and-kick --provider qwen` |
| Kimi | `quackd[kimi]` | `MOONSHOT_API_KEY` | `uvx --from "quackd[microduck,kimi]" quackd run find-and-kick --provider kimi` |
| GLM | `quackd[glm]` | `ZAI_API_KEY` | `uvx --from "quackd[microduck,glm]" quackd run find-and-kick --provider glm` |
| Meta | `quackd[meta]` | `META_API_KEY` | `uvx --from "quackd[microduck,meta]" quackd run find-and-kick --provider meta` |
| fake (scripted) | none | none | `uvx --from "quackd[microduck]" quackd run find-and-kick --provider fake` |
| Ollama (local) | `quackd[openai]` | none | `uvx --from "quackd[microduck,openai]" quackd run find-and-kick --provider ollama --model qwen3:8b` |
| vLLM (local) | `quackd[openai]` | none | `uvx --from "quackd[microduck,openai]" quackd run find-and-kick --provider vllm --model Qwen/Qwen3-8B` |
| llama.cpp (local) | `quackd[openai]` | none | `uvx --from "quackd[microduck,openai]" quackd run find-and-kick --provider llamacpp` |
| LM Studio (local) | `quackd[openai]` | none | `uvx --from "quackd[microduck,openai]" quackd run find-and-kick --provider lmstudio` |
| any OpenAI compatible server | `quackd[openai]` | optional | `uvx --from "quackd[microduck,openai]" quackd run find-and-kick --provider local --base-url http://host:8000/v1` |

Every row above runs the cartoon, because `quackd[microduck]` is the body those lines ask for and `find-and-kick` names no robot of its own. To put the same model on the physics simulator instead, swap that extra for `quackd[mujoco]`, which is the same duck with MuJoCo behind it, and name the backend: `uvx --from "quackd[mujoco,anthropic]" quackd run find-and-kick --provider anthropic --robot microduck:mujoco`. The extras are independent, so `quackd[anthropic]` alone gives you a brain and no body at all. Nobody stands in the physics arena, so `follow-me`, whose whole task is to follow somebody, cannot succeed there and nothing stops you pointing it at that backend anyway.
...
**The stepper is not in that table on purpose.** `--jev` is not a provider and `--provider` does not take it: it answers typed questions about a state and writes nothing, so it cannot pilot a robot on its own. It sits in front of whichever provider you did pick, for the turns whose answer is a choice, and it is off unless you ask for it ([docs/jev.md](docs/jev.md)).

| Command | What it does |
|---|---|
| `quackd run <duck>` or `quackd run --goal "..."` | Run a task. `--provider` picks the model, `--robot` the body (a spec or a registered name), `--flock` runs several at once, `--dry-run` sends nothing, `--image` hands the task a picture and repeats, `--by-hand` lets you place the arm where the run starts, `--jev` puts an optional discrete stepper in front of the model, `--run-name` labels the run directory and is what `quackd trace` finds it by afterwards, `--price` says what the model costs in USD per million tokens (`in=3,out=15[,cache_read=0.3,cache_write=3.75]`) when quackd has no published rate for it or yours is negotiated, `--live` opens a window, `--no-trace` stops it narrating. `quackd run --help` has the rest, grouped. It exits 1 when a run does not succeed, and 3 when the pilot judged the task beyond this body and nothing moved |
...
| `quackd record <duck>` | `run` pinned to `microduck:sim2d` (no `--robot`, so it wants `quackd[microduck]`) that always writes a GIF. `--seed` defaults to 0 and gated verbs are auto accepted, as with `--yes`. `--no-trace` and `--no-trace-prompt` work here too |
| `quackd trace [run]` | Replay a finished run from its transcript, on stdout, as the same lines it printed while it ran. No argument means the newest run under `--runs-dir`, and a task name, a timestamp prefix, the name you gave the run with `--run-name` (`quackd trace example-1`, which beats a longer directory that merely contains the text) or a transcript file all work. `--no-prompt`, `--thinking all|N`, `--from-step N`, `--frames` |

### Your robots, by name

Reaching a real robot takes a spec, an address, a token and a camera URL, and retyping them on every command puts that token in your shell history. `quackd robot add` keeps them instead: which body, where it is, its token and its cameras, where an arm should be left when a run ends, and optionally the provider and model that pilot it. A flock is a named list of those entries. Register once, and `--robot NAME` means the same thing in every command that takes a robot, `--flock NAME` in `run` and `serve-mcp`.
...
| `arm-grip-check` | say whether the arm is holding anything, prove it with the gripper, then let go | the same **SO-101 arm**, one step further: it reads, grips and releases, and authors no angle at all, because `move_joints` is deliberately not in its allowlist. It is the task that asks the question nobody has answered on hardware, whether the band that infers `holding` from a gripper stopping short of shut is right, and it is the worked example in [docs/jev.md](docs/jev.md) because every turn in it is a choice. Run it with `--by-hand` and put something in the gripper |
...
uvx --from "quackd[open_duck]" quackd run open-duck-scout --provider fake         # it finds the ball and walks up, 10 of 10 seeds
...
```bash
uvx --from "quackd[microduck,lerobot]" quackd run flock-hello --provider fake             # the bundled demo: a duck and an arm, no key and no registry
uvx --from "quackd[microduck,lerobot,anthropic]" quackd run <duck> --flock pair --provider anthropic   # your own robots, from the registry, one extra per body
```
...
| Discrete stepper | `--jev off` (the default), `shadow` or `on`, or `QUACKD_JEV`, and the flag wins. Needs `quackd[jev]` and `TYPESAFE_API_KEY`. With either missing the run says so once before connecting and carries on without the stepper, because the model is the pilot either way. A mode that is not one of the three is a typo and stops the run. `shadow` records what it would have chosen beside what the model did and changes nothing about the run. `on` lets it answer the turns whose answer is a choice among calls this body already has, and every pose, every sentence and every feasibility verdict stays the model's. `TYPESAFE_DEFAULT_MODEL` picks the model, default `jev-1.13.0`. See [docs/jev.md](docs/jev.md) |
...
On the simulator with the scripted pilot, `find-and-kick` takes 3 to 8 verb steps, one model call each plus one to declare success, and under a second of loop wall clock per run on a laptop. Interpreter start and GIF rendering add a few seconds to the whole command, and simulated time runs as fast as the CPU allows. With a real model each decision is one API call: the system prompt and the tool schemas are about 7 k characters (roughly 2 k tokens) with memory on, each observation a few hundred characters plus a 256 px PNG for vision models, and the transcript records each provider's own usage per turn. With `--jev on` a turn whose answer is a choice among calls this body already has can be answered by a classifier instead of by the model, and every pose and every sentence is still one API call. TypeSafe publish 0.114 seconds and $0.042 per million input tokens for Jev, so against the 6.21 second mean model call measured on the arm above, one of those turns is roughly 54 times faster and costs roughly a two-thousandth as much, because the
stepper is both cheaper per token and asked a far smaller question than the model is. What a whole run saves is capped by how many of its turns are a choice rather than a number, which is a property of the task: roughly 1.2 times faster on the wave run above, where one turn in five qualifies, and roughly 2.9 times on `arm-grip-check`, where a measured run put four turns of six on the stepper. **Those are estimates, not measurements**, and [docs/jev.md](docs/jev.md) shows the workings, names every input and gives a recipe for replacing them with numbers from your own bench. Model latency never affects control, because the steering loop runs at 10 Hz and the robot's own controllers run regardless of how long the model thinks. That holds for local models too. The core install is about 250 MB, needs no GPU, and the simulator renders at 256 px (`--gif-size` for prettier GIFs).
...
- The optional discrete stepper is new, off by default, and has never driven hardware. It is text only, so no camera frame reaches it, it authors no number and no sentence, and it cannot end a run or record a feasibility verdict. How much it saves depends entirely on how many of a task's turns are a choice rather than a number, which is a property of the task and the body and not of Jev: on the wave run at the top only two of the ten calls are the kind it can answer, and on `arm-grip-check` a measured run put four turns of six on it. [docs/jev.md](docs/jev.md) works both out, says plainly that they are estimates, and carries a recipe for replacing them with measurements from your own bench.
