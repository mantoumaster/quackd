...
- Each joint's travel in degrees, read from its own calibration, and the only goals quackd
  ...
  wrist_flex -103.2 to 103.2, wrist_roll -180 to 180, gripper 0 to 100. A joint can read past
  its travel when it was folded or placed there with torque off, which is where a rest pose
  usually is; goals are still limited to the travel.
...
Version 0.13, one real arm, two simulators and mocks for the rest. What has been built, and how far each piece has actually been exercised:
...
**This changes what a probe and a dry run leave behind.** On an arm that is away from its recorded rest pose, torque is now left ON where it used to be dropped: the arm holds itself up instead of sagging, and it stays that way until you hold it and run `quackd robot release NAME`, let `quackd doctor --robot NAME` park it, or cut its power.
