...
- Nobody has run quackd on an NVIDIA Jetson. The image in [`deploy/jetson/`](https://github.com/rokbenko/quackd/tree/v0.13.0/deploy/jetson) was built for arm64 and run under emulation on a Windows laptop, and built and run again on a native arm64 CI runner with no GPU, green on 2026-09-23. Between them they prove quackd runs on aarch64 Linux and prove nothing about the board: not Ollama on its GPU, not the NVIDIA container runtime, and not whether a model server and a robot's control loop can share one board without the loop suffering, which on a humanoid is a fall.
