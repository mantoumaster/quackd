...
| Discrete stepper (`--jev`, `quackd[jev]`) | ✅ implemented and tested offline against a stub SDK, with the classification of every verb of every shipped body frozen in a test, 🧪 never run against TypeSafe's API and never on hardware, so there is no latency figure and no agreement rate here. Off by default. `--jev shadow` exists to measure it before anybody switches it on ([docs/jev.md](docs/jev.md)) |
...
- The optional discrete stepper is new, off by default, and has never driven hardware. It is text only, so no camera frame reaches it, it authors no number and no sentence, and it cannot end a run or record a feasibility verdict. TypeSafe publish no latency figure, so this page claims no speed result for it: on the wave run at the top only two of the ten calls are the kind it can answer, and [docs/jev.md](docs/jev.md) carries the arithmetic and a recipe for measuring it rather than a number.
