...
| Letting go | Torque is released only where the arm is known to be at that pose, or as near it as its calibration lets the servos go. Where it is not, quackd turns LeRobot's own flag off, leaves the arm holding itself up, and prints one line: `the arm is not at its rest pose (...), so torque was left on and it will not fall: hold the arm and cut its power, or run again` |
