...
|---|---|
| `quackd run <duck>` or `quackd run --goal "..."` | Run a task. `--provider` picks the model, `--robot` the body (a spec or a registered name), `--flock` runs several at once, `--dry-run` sends nothing, `--image` hands the task a picture and repeats, `--by-hand` lets you place the arm where the run starts, `--jev` puts an optional discrete stepper in front of the model, `--live` opens a window, `--no-trace` stops it narrating. `quackd run --help` has the rest, grouped. It exits 1 when a run does not succeed, and 3 when the pilot judged the task beyond this body and nothing moved |
