...
The extras are independent and they compose, so a body and a brain are one install (`uv pip install "quackd[microduck,anthropic]"`) or none at all (`uvx --from "quackd[microduck,anthropic]" quackd run ...`). `quackd[lerobot]` needs Python 3.12 or newer, because LeRobot does, and `quackd[yolo]`, `quackd[live]` and `quackd[lan]` are the other detector, the live window and LAN discovery. Which body a command means when you do not name one is in [Configuration](#configuration), and `quackd doctor` says what this machine has.
