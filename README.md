...
| Discrete stepper | `--jev off` (the default), `shadow` or `on`, or `QUACKD_JEV`, and the flag wins. Needs `quackd[jev]` and `TYPESAFE_API_KEY`, and is refused before the robot is connected when either is missing. `shadow` records what it would have chosen beside what the model did and changes nothing about the run. `on` lets it answer the turns whose answer is a choice among calls this body already has, and every pose, every sentence and every feasibility verdict stays the model's. `TYPESAFE_DEFAULT_MODEL` picks the model, default `jev-1.13.0`. See [docs/jev.md](docs/jev.md) |
