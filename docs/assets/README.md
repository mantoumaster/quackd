...
says so. The transcripts are the exception, and the only real model runs in this directory: four
of them, two contributors, two local servers, no API key and no frame between them. Nothing here
has yet been recorded with a cloud model.
