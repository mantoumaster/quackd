...
| `release` | only at the end of a run whose last rest move missed, with a person at the terminal: the offer to take torque off the arm while they hold it, which the run makes between that rest move and the close. `stage` is `released` (Enter came and the arm was asked to let go where it stands, with `how` the arm's own word, `released` or `refused`, the `joints` it was at, and `torque_on`, the joints whose torque still read on afterwards: empty when every one read off, null when nothing was read back) or `kept` (the wait ran out, nothing could be pressed, or a Ctrl-C landed on it, so torque stays on as it would have without the offer, and `reason` says which). Never on a dry run, an MCP session or a flock member ([adapters/lerobot.md](adapters/lerobot.md#the-torque-rule)) |
